...
import { FaEdit, FaRandom, FaUsers } from 'react-icons/fa'
...
function generatePoolFixtures(poolA, poolB, date, time, format) {
  const matches = []
  ...
  return matches.map((fixture) => ({ ...fixture, format }))
}
...
function PoolFixtures() {
  ...
  if (!isPoolStage) return <Fixtures />

  const getTeam = (id) => teams.find((team) => team.id === id)
  const stageReady = poolFixtures.length === 9
  const completedCount = poolFixtures.filter((fixture) => fixture.status === 'Completed' && fixture.winnerId).length
  ...
  const saveEdit = async () => {
    ...
    const winnerId =
      draft.status === 'Completed'
        ? homeScore > awayScore ? editing.homeTeamId : editing.awayTeamId
        : ''
  ...
  const renderPool = (name, pool) => (
    <section className="glass-card" key={name}>
      ...
      <div style={{ display: 'grid', gap: 8 }}>
        {pool.map((team, index) => (
          <div key={team.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 12px', border: '1px solid rgba(255,255,255,.06)', borderRadius: 8 }}>
  ...
  return (
    <section className="fixtures-page">
      ...
      <header className="fixtures-heading">
        ...
        {isAdmin && !isSuperAdmin && <div className="fixtures-readonly-note">Team Admin · Read Only</div>}
        {isSuperAdmin && (
          <button className="button button-primary" onClick={() => setIsGeneratorOpen(true)}>
            <FaRandom /> Generate Pool Stage
          </button>
        )}
      </header>
      ...
      {poolFixtures.length === 0 ? (
        <section className="glass-card playoff-empty">
          <FaRandom />
          <h2>No pool fixtures generated</h2>
          <p>Generate the 9 cross-pool BO1 fixtures to begin the tournament stage.</p>
        </section>
      ) : (
            ...
            <section className="glass-card" key={round}>
              ...
              <div className="pool-fixture-grid">
                {poolFixtures.filter((fixture) => fixture.round === round).map((fixture) => {
                  ...
                  return (
                    <article className={`pool-fixture-card ${completed ? 'pool-fixture-done' : ''}`} key={fixture.id}>
                      ...
                      <div className="pool-fixture-bottom">
                        ...
                        <span>{completed ? `Winner: ${getTeam(fixture.winnerId)?.name || 'TBD'}` : 'Upcoming'}</span>
                        {isSuperAdmin && (
                          <button className="icon-button" type="button" onClick={() => openEdit(fixture)} aria-label="Edit match">
                            <FaEdit />
                          </button>
                        )}
                      </div>
      ...
      {stageReady && (
        <section className="glass-card" style={{ marginTop: 16 }}>
          ...
          <p style={{ margin: '6px 0 0' }}>When all 9 are complete, the top 2 teams from Pool A and Pool B can advance to the Semi Finals.</p>
        </section>
      )}
          ...
          <section className="team-modal fixture-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <form onSubmit={generate}>
              ...
              <div className="modal-actions">
                ...
                <button className="button button-primary" type="submit">Generate 9 Fixtures</button>
              </div>
            </form>
          </section>
        </div>
      ...
      {isSuperAdmin && editing && draft && (
        <div className="modal-backdrop" onMouseDown={() => setEditing(null)}>
          <section className="team-modal fixture-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <label>Status<select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value })}><option value="Upcoming">Upcoming</option><option value="Completed">Completed</option></select></label>
            <div className="modal-actions">
